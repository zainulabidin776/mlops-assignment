...
class APITester:
    """Comprehensive API testing class for Windows"""
    ...
    def start_flask_app(self) -> bool:
        ...
        try:
            ...
            # Start Flask app in background using Windows method
            ...
            print(f"{Colors.CYAN}⏳ Waiting for Flask app to start...{Colors.NC}")
            time.sleep(8)
            
            # Check if process is running
            if self.flask_process.poll() is None:
                self.print_test("Flask App Start", "PASS", f"Application started successfully (PID: {self.flask_process.pid})")
                return True
            else:
    ...
    def test_health_endpoint(self) -> bool:
        ...
        try:
            print(f"{Colors.CYAN}🌐 Testing health endpoint...{Colors.NC}")
    ...
    def test_prediction_endpoint_valid(self) -> bool:
        ...
        try:
            print(f"{Colors.CYAN}🔮 Testing prediction endpoint with valid data...{Colors.NC}")
            # Valid test data (using correct feature names from metadata.json)
            test_data = {
                "Age": 65,
                "Sex": 1,
                "ChestPainType": 3,
    ...
    def test_prediction_endpoint_missing_features(self) -> bool:
        ...
        try:
            print(f"{Colors.CYAN}❌ Testing prediction endpoint with missing features...{Colors.NC}")
            # Incomplete test data (using correct feature names)
            test_data = {
                "Age": 65,
                "Sex": 1
    ...
    def test_prediction_endpoint_invalid_json(self) -> bool:
        ...
        try:
            print(f"{Colors.CYAN}🔧 Testing prediction endpoint with invalid JSON...{Colors.NC}")
            response = requests.post(
                f"{self.base_url}/predict",
    ...
    def test_prediction_endpoint_wrong_method(self) -> bool:
        ...
        try:
            print(f"{Colors.CYAN}🚫 Testing prediction endpoint with wrong method...{Colors.NC}")
            response = requests.get(f"{self.base_url}/predict", timeout=15)
    ...
    def test_prediction_endpoint_empty_data(self) -> bool:
        ...
        try:
            print(f"{Colors.CYAN}📭 Testing prediction endpoint with empty data...{Colors.NC}")
            response = requests.post(
                f"{self.base_url}/predict",
                json={},
                headers={'Content-Type': 'application/json'},
    ...
    def test_model_loading(self) -> bool:
        ...
        try:
            print(f"{Colors.CYAN}🤖 Testing model files...{Colors.NC}")
    ...
    def test_app_imports(self) -> bool:
        ...
        try:
            print(f"{Colors.CYAN}📦 Testing app imports...{Colors.NC}")
    ...
    def test_response_time(self) -> bool:
        ...
        try:
            print(f"{Colors.CYAN}⏱️  Testing response time...{Colors.NC}")
    ...
    def run_all_tests(self) -> Tuple[int, int, int]:
        ...
        self.print_header("🧪 Flask API Comprehensive Testing for Windows")
        ...
        print(f"{Colors.CYAN}⏳ Waiting for API to be ready...{Colors.NC}")
        time.sleep(3)
...
def main():
    ...
    print(f"{Colors.PURPLE}🧪 Flask API Test Script for Windows - MLOps Assignment{Colors.NC}")
    ...
    try:
        ...
        print(f"{Colors.YELLOW}💡 Run: pip install requests{Colors.NC}")
        input("Press Enter to continue after installing requests...")
        sys.exit(1)
    ...
    try:
        ...
        print(f"{Colors.CYAN}Press Enter to exit...{Colors.NC}")
        input()
        sys.exit(1)
    except Exception as e:
